...
$totalSnps->execute([$patientId]);$snpCount=$totalSnps->fetchColumn();
...
<tr><td class='text-muted'>Genotipo</td><td><strong class='fs-5'><?=$r['patient_genotype']??'N/D'?></strong></td></tr>
<tr><td class='text-muted'>Fenotipo</td><td class='<?=$bgClass?> rounded px-2'><?=sanitize($r['phenotype']??'Nao avaliado')?></td></tr>
<tr><td class='text-muted'>Evidencia</td><td>
...
<strong>Recomendacao:</strong> <?=sanitize($r['interpretation']??$r['recommendations']??'')?>
</div>
<?php endif;?>
</div></div></div>
<?php endforeach;?>
