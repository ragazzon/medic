...
$pdo=getConnection();
$p=$pdo->prepare('SELECT * FROM patients WHERE id=?');$p->execute([$patientId]);$patient=$p->fetch();
$summary=getGenomicSummary($patientId);
if(!$summary){setFlash('info','Sem dados genomicos.');redirect(baseUrl('pages/genomic/upload.php?patient_id='.$patientId));}
...
<div class='page-header'>
    <h1><i class='bi bi-card-checklist me-2'></i>Analise Genomica - <?=sanitize($patient['name'])?></h1>
    <div>
        <a href='<?=baseUrl('pages/genomic/argue.php?patient_id='.$patientId)?>' class='btn btn-outline-danger btn-sm'><i class='bi bi-chat-left-quote me-1'></i>Argumente com o Medico</a>
...
<!-- ===== SECAO 1: MEDICAMENTOS ===== -->
<div class='card mb-4'>
    <div class='card-header bg-primary text-white'>
        <h5 class='mb-0'><i class='bi bi-capsule me-2'></i>Analise de Medicamentos por Categoria</h5>
    </div>
    <div class='card-body'>
        <p class='text-muted mb-3'>Como o organismo metaboliza cada medicamento com base no perfil genetico.</p>
        ...
        <div class='row g-3 mb-3'>
            ...
            <div class='col-md-6 col-lg-4'>
                <div class='card h-100 border-start border-4 <?=$d['worst_status']==='risk'?'border-danger':($d['worst_status']==='attention'?'border-warning':'border-success')?>'>
                    <div class='card-body'>
                        <div class='d-flex justify-content-between align-items-start mb-2'>
                            <h6 class='mb-0'><?=sanitize($d['name'])?></h6>
                        ...
                        <div class='mt-2'>
                            ...
                            <div class='d-flex justify-content-between align-items-center py-1 border-top'>
                                <div>
                                    <small><strong><?=$g['gene_symbol']?></strong></small>
                                ...
                                <div class='text-end'>
                                    ...
                                    <small class='fw-bold'><?=$g['patient_genotype']??'N/D'?></small>
                                </div>
                            </div>
                            <?php endforeach;?>
                        </div>
        ...
        <!-- Sub-paineis de medicamentos -->
        <h6 class='mt-4 mb-2'><i class='bi bi-grid me-1'></i>Paineis Detalhados de Metabolismo</h6>
        <div class='row g-2'>
...
<!-- ===== SECAO 2: RISCOS DE DOENCAS ===== -->
<div class='card mb-4'>
    <div class='card-header bg-danger text-white'>
        <h5 class='mb-0'><i class='bi bi-shield-exclamation me-2'></i>Riscos Geneticos para Doencas</h5>
    </div>
    <div class='card-body'>
        <p class='text-muted mb-3'>Predisposicoes geneticas identificadas. NAO significam que a doenca vai ocorrer.</p>
        <div class='row g-3'>
